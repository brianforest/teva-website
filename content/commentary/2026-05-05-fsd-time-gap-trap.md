...
>
> 「應」就是強制義務（mandatory requirement），等同於「必須」、「須」。沒拿到 Article 39 認證之前根本不可能補齊文件。所以 4/10 之前根本不能遞件，這不就是法規白紙黑字寫死的嗎？

這個質疑的字面解讀完全正確——「應」確實是強制義務。但仔細看，它規範的對象只有一個：**廠商**。

「應提出佐證文件」是**廠商正式遞件時的文件清單要求**——它規範的是「正式遞件這個動作，必須附上什麼」。
