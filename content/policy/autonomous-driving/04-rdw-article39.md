...
**第二條路：有條件型式認證。** 仿照 RDW 模式，在台灣建立「有條件 DCAS 型式認證」機制——車廠提交全球安全數據加上台灣道路模擬測試結果，VSCC 發放有條件認證，限定期限，要求回報運行數據。需要修訂《車輛型式安全審驗管理辦法》，但仍在部會命令層級，不需要立法院。

兩條路都有技術方案。每一條路的法律障礙都是可解的。
