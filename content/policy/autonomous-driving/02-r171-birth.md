...
released: false
publish_date: "4月16日"
release_date: 2026-04-16T00:00:00+08:00
...
接下來的六年，國際社會將花上無數場會議、無數次妥協、無數個深夜的條文拉鋸，去完成一件事：**把 AI 駕駛行為從 R79 裡分家出來，給它一個獨立的家。**
...
R171 引入了一個我們稱為「**論證雛形**」的機制——這是本會給 R171 中分散在 5.3.7.1.3、5.3.6.2、Annex 附件 3 等條款的共同精神所取的整合名稱：**廠商把自己的「策略文件」交給主管機關（Type Approval Authority 型式認證主管機關），由主管機關審查策略合理性。**

什麼是「策略文件」？廠商必須說明：系統怎麼決定速度、怎麼處理彎道收緊、邊界條件下怎麼退場、lane positioning 車道內定位的策略是什麼。這些不是測試結果，是廠商的「設計邏輯」。

主管機關審查的，不只是某個瞬間的數字（「你超過 3 m/s² 了嗎？」），更是這份策略文件本身（「你說的這套策略，合不合理？能達成 R171 多項要求嗎？」）。

舉個實際情境。Tesla 申請 R171 認證時，可能會在策略文件中寫：

> 「FSD 進入半徑 100 公尺的彎道時，系統根據即時感測 + 神經網路判斷，選擇速度 v ≤ 50 km/h（對應側向加速度 ≤ 2 m/s²），並確保 manoeuvre 駕駛操作連續、可預測、駕駛人可控（5.3.7.2.1.5-12）。」

主管機關不檢查 Tesla 哪一秒超過了，而是審查這套策略合不合理、做不做得到。

這就是 **Safety Case 安全論證**精神在 R171 中的初步冒芽——廠商提論證，主管機關審論證合理性。

**為什麼需要這一層？**

純規則式（R79）只看單點數字，管不到 AI 的動態決策。AI 每一秒都在變，不是寫死的固定參數。一個會學習、會判斷的系統，不能只用「你超過上限了嗎？」這種瞬時檢查方式評估——你要看它整套策略合不合理。

**突破性**：主管機關的工具箱從「單點檢查」升級到「策略審查」——這是從純規則式往安全論證過渡的中間形態。

但**「雛形」就是雛形**：R79 那條 3 m/s² 的硬上限，R171 一字不漏完整保留（00/01/02 三個 Series 都還在）。場景框架（高速公路 / 類高速公路）是另一道枷鎖。R171 只是在「規則式底線」上探出半步——真正的完整 Safety Case 安全論證，要等到 Article 39 與下一代 L3+ ADS GTR 跨出去。

這就是所謂的 **「擬人化」**——讓機器駕駛的行為，接近一個合格的人類駕駛。

聽起來像常識。但在法規的世界裡，這是規則式工具箱探出的**半步**——往安全論證的方向。

三十年來，車輛法規的思維主要是：**機器是危險的，我們的工作是限制它。** 用數字限制。用紅線圍堵。用禁止來管控。

R171 **沒有取代**這個思維。它在硬數字底線之上，**加上一層**——讓廠商說明自己的策略，讓主管機關審查策略合理性。原本「不准機器自己動」的紅線並沒有消失：3 m/s² 還在，5 m/s³ 還在，場景限制還在。但 R171 在這條紅線上方，新增了一塊「論證雛形」的空間。

這是規則式法規史上第一次，主管機關的審查工具從「單點檢查」升級到「策略審查」。半步，但很關鍵。

從「不准機器動」到「也問一句：你的策略合理嗎？你會接嗎？」——這是工具箱的局部升級，還不是哲學的全面改寫。
...
2024 年，FSD 是 V12，正在從規則驅動全面轉向端到端神經網路。2025 年是 V13，城市道路能力大幅提升。2026 年是 V14。到 2032 年台灣 R171 Phase 1 全面適用時，FSD 可能已經在 V20 或更高的版本上了。

R171 Phase 1 的能力邊界——不允許系統主動變道、不允許脫手——到 2032 年會是什麼概念？那就像在 2032 年強制你的手機只能打電話和發簡訊，因為法規還停留在 2012 年的功能型手機時代。
